    ...
    def __init__(self):
        ...
        self.role_permissions = {
            ...
            UserRole.ANALYST: {
                ...
                "accessible_tables": ["employees", "departments", "products", "sales"],  # Limited tables
                "restricted_columns": {
            ...
            UserRole.VIEWER: {
                "permissions": ["read"],
                "accessible_tables": ["departments", "products"],  # Very limited
                "restricted_columns": {
                    "employees": ["salary", "manager_id", "hire_date"],  # Heavy restrictions
...
class FileDataManager:
    ...
    def _execute_file_query(self, sql_query: str, username: str, role_manager: 'RoleManager') -> pd.DataFrame:
        ...
        # Apply role-based column restrictions
        # Note: This is simplified - in practice you'd need to identify which table the results came from
        return df
    
    def _execute_db_query(self, sql_query: str, username: str, role_manager: 'RoleManager') -> pd.DataFrame:
        """Execute query on database sources"""
    ...
    def _execute_cross_source_query(self, sql_query: str, username: str, role_manager: 'RoleManager') -> pd.DataFrame:
        ...
        # This is a complex case that would require query parsing and rewriting
        # For now, we'll return an error message
        raise Exception("Cross-source queries not yet implemented. Please query database and files separately.")
    ...
    def _generate_fallback(self, query_description: str, user_role: UserRole = None, role_manager: 'RoleManager' = None) -> str:
        ...
        if user_role == UserRole.VIEWER:
            ...
            if "department" in desc:
                ...
                return "SELECT name FROM departments LIMIT 10;"
            ...
            if "count" in desc and "department" in desc:
                ...
                return "SELECT 'Salary data restricted for analysts' as message;"
        ...
        if role_manager:
            ...
            for table in query_tables:
                if not role_manager.check_table_access(username, table):
                    return f"Access denied: You don't have permission to access table '{table}'"
        
        try:
            print(f"🔍 Executing SQL on database: {actual_db_path}")
            
            # Check if query involves file sources
            if file_manager:
                try:
            ...
            if role_manager and len(df) > 0:
                ...
                if len(query_tables) == 1:
                    ...
                    print(f"🔒 Applied role-based column restrictions for table: {table_name}")
...
[Full data with {len(df)} rows and {len(df.columns)} columns has been retrieved and stored for export]"""
    ...
    def _extract_table_names(self, sql_query: str) -> List[str]:
        ...
        known_tables = ['employees', 'departments', 'products', 'sales']
