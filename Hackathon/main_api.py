...
- Enhanced feedback with conversation history
"""
...
def get_conversation_history_context(user_id: str) -> str:
    """Extract conversation history for better context-aware feedback"""
    ...
    context_lines = ["=== CONVERSATION HISTORY CONTEXT ==="]
    ...
    for i, msg in enumerate(recent_messages, 1):
        ...
        context_lines.append(f"User Request: {msg.message}")
        context_lines.append(f"Assistant Response: {msg.response}")
        context_lines.append(f"Success: {msg.success}")
        if msg.sql_query:
            context_lines.append(f"SQL Query: {msg.sql_query}")
...
async def websocket_endpoint(websocket: WebSocket, user_id: str):
    ...
    await safe_send_message(user_id, {
        ...
        "current_config": manager.user_sessions[user_id]['config'].model_dump(),
        "timestamp": datetime.now().isoformat()
    })
...
async def handle_chat_message(user_id: str, message_data: dict):
    ...
    try:
        ...
        if result.get("success"):
            ...
            # Handle validation errors specially - don't execute, just show the error
            print(f"❌ Validation error detected")
            response_data.update({
                "response": "❌ The generated SQL query has validation errors and cannot be executed.",
                "validation_error": True,
                "validation_issues": result.get("validation_issues", ["Unknown validation error"]),
                "bad_sql": result.get("bad_sql", ""),
                "sql_query": result.get("bad_sql", ""),  # Set the bad SQL for feedback
                "row_count": 0,
                "columns": [],
                "data": []
            })
            ...
            # Try to extract the actual database error if available
            if 'Execution failed' in error_msg and 'sql' in error_msg:
                try:
                    if ': ' in error_msg:
                        parts = error_msg.split(': ')
                        if len(parts) > 2:
                            actual_error = parts[-1]  # Get the last part which is the actual error
                            error_msg = f"Database error: {actual_error}"
                except:
            ...
            response_data.update({
                ...
                "error": error_msg,
                "sql_query": result.get("sql_query", ""),  # Include the failed SQL for feedback
                "row_count": 0,
                "columns": [],
                "data": []
            })
        ...
        # Store in chat history - ALWAYS store, even for failed queries
        ...
        # Always append to chat history, regardless of success/failure
        manager.user_sessions[user_id]['chat_history'].append(chat_message)

        # Set message_id in response for feedback tracking
...
async def handle_feedback(user_id: str, message_data: dict):
    """Enhanced feedback handler that considers all conversation history"""
    try:
        ...
        # Get conversation history context for better feedback processing
        conversation_context = get_conversation_history_context(user_id)
        
        print(f"🔄 Processing feedback with conversation context for user {user_id}")
        print(f"📝 Feedback: {feedback}")
        print(f"📚 Conversation context length: {len(conversation_context)} characters")
        
        # Enhanced feedback with conversation history
        enhanced_feedback = f"""
        ORIGINAL USER REQUEST: {original_message.message}
        
        USER FEEDBACK: {feedback}
        
        {conversation_context}
        
        INSTRUCTIONS: Use the above conversation history to understand the user's intent better and provide a more contextually aware response that addresses their feedback while considering all previous interactions.
        """
        
        # Apply feedback with enhanced context
        previous_result = {
            ...
            "conversation_history": chat_history,  # Include full conversation history
            "original_sql": original_message.sql_query  # Include the original SQL for reference
        }
        
        result = app_instance.system.apply_feedback_and_retry(
            previous_result=previous_result,
            feedback=enhanced_feedback,  # Use enhanced feedback with context
            username=user_id,
            create_chart=message_data.get("create_chart", False),
            chart_type=message_data.get("chart_type", "bar")
        )
        ...
        if result.get("success"):
            ...
            if execution_data:
                if "data" in execution_data:
                    raw_data = execution_data["data"]
                    data_rows = safe_json_serialize(raw_data) if raw_data else []
                if "columns" in execution_data:
                    columns = execution_data["columns"] or []
                if "sql_query" in execution_data:
                    sql_query = execution_data["sql_query"] or ""
            
            response_data.update({
                "response": "🔄 Feedback applied successfully with conversation context! The AI has considered your previous interactions to provide a better response.",
                "sql_query": sql_query,
                "row_count": execution_data.get("row_count", 0),
                "columns": columns,
                "data": safe_json_serialize(data_rows[:100]) if data_rows else []
            })
            
            # Store the improved response in chat history
            improved_message = ChatMessage(
                id=new_message_id,
                user_id=user_id,
                message=f"[FEEDBACK APPLIED] {original_message.message}",
                response=response_data["response"],
                timestamp=datetime.now(),
        ...
        print(f"✅ Feedback processing completed for user {user_id}")
...
async def handle_config_update(user_id: str, message_data: dict):
    ...
    try:
        ...
        # --- ADD THIS LOG ---
        print(f"✅ Re-initializing CrewAI app for user {user_id} with databases: {config.selected_databases}")
        ...
        await safe_send_message(user_id, {
            ...
            "message": f"Configuration updated. Now querying {len(config.selected_databases)} database(s) as {config.user_role}.",
            "timestamp": datetime.now().isoformat()
        })
...
async def health_check():
    return {
        ...
        "features": [
            ...
            "Enhanced Conversation History Feedback"
        ]
    }

if __name__ == "__main__":
    import uvicorn
    print("🚀 Starting Enhanced Gainwell SQL Assistant API...")
    ...
    print("   • Conversation history-aware feedback processing")
