...
class FileDataManager:
    ...
    def get_role_based_schema_info(self, username: str, role_manager: 'RoleManager') -> str:
        ...
        database_table_map = {}  # Track which tables exist in which databases
        ...
        for db_name in self.selected_databases:
            ...
            try:
                ...
                # Store actual tables that exist in this database
                database_table_map[db_name] = all_tables
                ...
                if accessible_tables:
                    ...
                    schema_info += f"DATABASE: {db_name}\n"
                    schema_info += f"No accessible tables found (Total: {len(all_tables)} tables)\n\n"
        ...
        # Store the database-table mapping for validation
        self.database_table_map = database_table_map
        # Store the database-table mapping for validation
        self.database_table_map = database_table_map
        ...
        # Add table existence validation rules
        schema_info += "7. CRITICAL: Only use tables that exist in the selected database(s)\n"
        schema_info += "8. If a table doesn't exist, return appropriate error message\n"
        
        if len(self.selected_databases) > 1:
            schema_info += "9. Multi-database environment - specify database context when needed\n"
        
        if user_role == UserRole.VIEWER:
            schema_info += "10. VIEWER RESTRICTIONS: No personal data, no salary info, no sensitive data\n"
        elif user_role == UserRole.ANALYST:
            schema_info += "10. ANALYST RESTRICTIONS: Limited personal data access\n"
        
        schema_info += "="*50 + "\n"
        
        return schema_info
    
    def validate_table_exists(self, table_name: str, target_database: str = None) -> tuple[bool, str]:
        """Validate if a table exists in the specified database or any selected database"""
        if not hasattr(self, 'database_table_map'):
            return False, "Database table mapping not initialized"
        
        if target_database:
            # Check specific database
            if target_database in self.database_table_map:
                tables = self.database_table_map[target_database]
                if table_name in tables:
                    return True, f"Table '{table_name}' found in database '{target_database}'"
                else:
                    available_tables = ", ".join(tables) if tables else "No tables"
                    return False, f"Table '{table_name}' does not exist in database '{target_database}'. Available tables: {available_tables}"
            else:
                return False, f"Database '{target_database}' not found in selected databases"
        else:
            # Check all selected databases
            found_in_databases = []
            for db_name, tables in self.database_table_map.items():
                if table_name in tables:
                    found_in_databases.append(db_name)
            
            if found_in_databases:
                return True, f"Table '{table_name}' found in database(s): {', '.join(found_in_databases)}"
            else:
                all_available_tables = set()
                for db_name, tables in self.database_table_map.items():
                    all_available_tables.update(tables)
                available_tables_str = ", ".join(sorted(all_available_tables)) if all_available_tables else "No tables"
                return False, f"Table '{table_name}' does not exist in any selected database. Available tables across all databases: {available_tables_str}"
    
    def get_available_tables_for_databases(self) -> Dict[str, List[str]]:
        """Get mapping of available tables for each database"""
        if hasattr(self, 'database_table_map'):
            return self.database_table_map
        return {db: [] for db in self.selected_databases}
    ...
    def execute_combined_query(self, sql_query: str, username: str, role_manager: 'RoleManager') -> pd.DataFrame:
        ...
        try:
            ...
            # Validate that requested tables exist
            requested_tables = self._extract_table_names_from_query(sql_query)
            for table in requested_tables:
                if table not in file_tables:  # Skip file tables validation here
                    table_exists, validation_msg = self.validate_table_exists(table)
                    if not table_exists:
                        table_validation_errors.append(validation_msg)
            
            # If we have validation errors, return them
            if table_validation_errors:
                error_df = pd.DataFrame([{
                    "error_type": "Table Not Found",
                    "error_message": "; ".join(table_validation_errors),
                    "requested_tables": ", ".join(requested_tables),
                    "available_databases": ", ".join(self.selected_databases)
                }])
                print(f"❌ Table validation failed: {table_validation_errors}")
                return error_df
    ...
    def _extract_table_names_from_query(self, sql_query: str) -> List[str]:
        ...
        # Known database tables to check for across all databases
        all_known_tables = set()
        if hasattr(self, 'database_table_map'):
            for db_tables in self.database_table_map.values():
                all_known_tables.update(db_tables)
        else:
            # Fallback to common table names
            all_known_tables = {'employees', 'departments', 'products', 'sales'}
        
        # Known file tables
        file_tables = [info['table_name'] for info in self.data_sources.values()]
        all_known_tables.update(file_tables)
        
        # Check for each known table in the query
        for table in all_known_tables:
            # Look for table name as whole word (not as part of another word)
            import re
...
7. CRITICAL: If the requested table doesn't exist in the accessible schema above, return an error message like:
   SELECT 'Error: Table [table_name] does not exist in selected database(s). Available tables: [list]' as error_message;
8. Only use tables that are explicitly listed in the accessible schema above
9. If no appropriate tables are found for the query, suggest available alternatives
...
Generate the SQL query now:"""
    ...
    def _generate_fallback(self, query_description: str, user_role: UserRole = None, role_manager: 'RoleManager' = None) -> str:
        """Role-aware fallback SQL generation with table existence validation"""
        desc = query_description.lower()
        
        # Get file manager for table validation
        file_manager = getattr(self, '_file_manager', None)
        ...
        if "product" in desc or "sales" in desc:
            if "product" in desc and "sales" in desc:
                ...
                if file_manager:
                    product_exists, product_msg = file_manager.validate_table_exists("products")
                    sales_exists, sales_msg = file_manager.validate_table_exists("sales")
                    
                    if not product_exists and not sales_exists:
                        return f"SELECT 'Error: Neither products nor sales tables exist in selected database(s). {product_msg}' as error_message;"
                    elif not product_exists:
                        return f"SELECT 'Error: Products table not found. {product_msg}' as error_message;"
                    elif not sales_exists:
                        return f"SELECT 'Error: Sales table not found. {sales_msg}' as error_message;"
                ...
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("products")
                    if not table_exists:
                        return f"SELECT 'Error: {msg}' as error_message;"
                ...
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("sales")
                    if not table_exists:
                        return f"SELECT 'Error: {msg}' as error_message;"
        ...
        if user_role == UserRole.VIEWER:
            ...
            if "department" in desc:
                ...
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("departments")
                    if not table_exists:
                        return f"SELECT 'Error: {msg}' as error_message;"
                return "SELECT name, location FROM departments LIMIT 10;"
            elif "employees" in desc:
                # Viewers should NOT access employees table, but also check if it exists
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("employees")
                    if not table_exists:
                        return f"SELECT 'Info: Employees table does not exist in selected database(s). {msg}' as info_message;"
                return "SELECT 'Access denied: Viewers cannot access employee data' as access_denied_message;"
            else:
                # Default to departments if available
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("departments")
                    if not table_exists:
                        # Try to find any available table for viewer
                        available_tables = file_manager.get_available_tables_for_databases()
                        viewer_accessible = ["departments", "products", "sales"]
                        found_table = None
                        for db_name, tables in available_tables.items():
                            for table in viewer_accessible:
                                if table in tables:
                                    found_table = table
                                    break
                            if found_table:
                        ...
                        if found_table:
                            if found_table == "products":
                                return "SELECT product_name, category FROM products LIMIT 10;"
                            elif found_table == "sales":
                                return "SELECT sale_date, quantity FROM sales LIMIT 10;"
                        
                        return f"SELECT 'Error: No accessible tables found in selected database(s). Available tables: {msg}' as error_message;"
                return "SELECT name, location FROM departments LIMIT 10;"
            ...
            if "count" in desc and "department" in desc:
                if file_manager:
                    emp_exists, emp_msg = file_manager.validate_table_exists("employees")
                    if not emp_exists:
                        return f"SELECT 'Error: Cannot count by department - {emp_msg}' as error_message;"
                ...
                if file_manager:
                    emp_exists, emp_msg = file_manager.validate_table_exists("employees")
                    if not emp_exists:
                        return f"SELECT 'Error: Cannot calculate salary averages - {emp_msg}' as error_message;"
                ...
                # Try employees first, then departments
                if file_manager:
                    emp_exists, emp_msg = file_manager.validate_table_exists("employees")
                    if emp_exists:
                        return "SELECT name, department FROM employees LIMIT 20;"
                    
                    dept_exists, dept_msg = file_manager.validate_table_exists("departments")
                    if dept_exists:
                        return "SELECT name, location FROM departments LIMIT 10;"
                    
                    return f"SELECT 'Error: No accessible tables found. {emp_msg}' as error_message;"
                return "SELECT name, department FROM employees LIMIT 20;"
        
        else:
            # Admin or fallback - full access but still check table existence
            if "count" in desc and "department" in desc:
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("employees")
                    if not table_exists:
                        return f"SELECT 'Error: Cannot count by department - {msg}' as error_message;"
                return "SELECT department, COUNT(*) as count FROM employees GROUP BY department ORDER BY count DESC LIMIT 10;"
            elif "average" in desc and "salary" in desc:
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("employees")
                    if not table_exists:
                        return f"SELECT 'Error: Cannot calculate salary averages - {msg}' as error_message;"
                ...
                if file_manager:
                    table_exists, msg = file_manager.validate_table_exists("employees")
                    if not table_exists:
                        return f"SELECT 'Error: Cannot query salary data - {msg}' as error_message;"
                return "SELECT name, department, salary FROM employees WHERE salary > 70000 ORDER BY salary DESC LIMIT 20;"
            else:
                # Default to employees if available, otherwise try departments
                if file_manager:
                    emp_exists, emp_msg = file_manager.validate_table_exists("employees")
                    if emp_exists:
                    ...
                    # Try product tables as last resort
                    prod_exists, prod_msg = file_manager.validate_table_exists("products")
                    if prod_exists:
                        return "SELECT product_name, category, price FROM products LIMIT 10;"
                    
                    return f"SELECT 'Error: No tables found in selected database(s). {emp_msg}' as error_message;"
                return "SELECT name, department, salary FROM employees LIMIT 10;"
