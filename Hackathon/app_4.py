...
with col1:
    ...
    db_folder = "."  # Or wherever your DBs are
    available_dbs = [f for f in os.listdir(db_folder) if f.endswith(".db")]

    # Fallback in case no DBs found
    if not available_dbs:
        available_dbs = ["No databases found"]
    with col_config1:
...
# with col_btn2:
#     # FIXED: More permissive condition for showing export button
#     show_export_button = False
    
#     if st.session_state.current_result:
#         # Check multiple conditions to show export button
#         has_execution_data = (
#             st.session_state.current_result.get('execution_data') is not None and 
#             st.session_state.current_result.get('execution_data', {}).get('data') is not None
#         )
        
#         # Also show if query was successful (fallback for admin users)
#         is_successful = st.session_state.current_result.get('success', False)
#         workflow_completed = st.session_state.current_result.get('workflow_completed', False)
        
#         # Show export button if ANY of these conditions are met
#         show_export_button = has_execution_data or (is_successful and workflow_completed)
        
#         # Additional check: if we have a crew_result that indicates data was processed
#         if not show_export_button and is_successful:
#             crew_result = st.session_state.current_result.get('crew_result', '')
#             if 'rows returned' in crew_result.lower() or 'query executed' in crew_result.lower():
#                 show_export_button = True
    
#     # Show export button or disabled version
...
#                 # First try: execution_data
#                 execution_data = st.session_state.current_result.get('execution_data')
#                 if execution_data and execution_data.get('data'):
#                     data_to_export = execution_data['data']
#                     st.info(f"📊 Exporting {len(data_to_export)} rows of data")
                
#                 # Second try: Check if system has stored data
#                 elif hasattr(app.system, 'last_execution_result') and app.system.last_execution_result:
#                     last_result = app.system.last_execution_result
#                     if last_result.get('data'):
#                         data_to_export = last_result['data']
#                         st.info(f"📊 Exporting {len(data_to_export)} rows from system cache")
                
#                 # Third try: Create sample export with metadata
#                 else:
#                     # Create a metadata-only export
#                     current_result = st.session_state.current_result
#                     metadata_export = [{
#                         'Export_Type': 'Query Metadata',
#                         'User': username,
#                         'User_Role': current_result.get('user_role', 'unknown'),
#                         'Query_Request': current_result.get('request', 'N/A'),
#                         'Workflow_Status': 'Completed' if current_result.get('success') else 'Failed',
#                         'Agents_Used': current_result.get('agents_used', 0),
#                         'LLM_Provider': current_result.get('llm_provider', 'Unknown'),
#                         'Timestamp': datetime.now().strftime('%Y-%m-%d %H:%M:%S')
#                     }]
#                     data_to_export = metadata_export
#                     st.info("📊 Exporting query metadata (no data results available)")
...
#                 if data_to_export and len(data_to_export) > 0:
#                     st.info(f"📊 Exported {len(data_to_export)} rows successfully")
                
#             except Exception as e:
#                 st.error(f"❌ Export failed: {str(e)}")
#                 st.info("💡 This might be due to no query results being available. Try running a query first.")
...
#             help="Run a successful query first to enable data export"
#         )
        
#         # Show helper text for admin users
#         if username == "admin" and st.session_state.current_result:
#             st.caption("💡 Export will be enabled after running a query that returns data")
